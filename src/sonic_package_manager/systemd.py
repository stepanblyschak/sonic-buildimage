    ...
    def _generate_systemd_service(self, package):
        ...
        for servicelist, services in dict(
            ...
            for service in service_props.get(servicelist, []):
                services.append(
                    dict(name=service, is_package=self._database.has_package(service))
                )

        SystemdGenerator.render_template(self.SERVICE_FILE_TEMPLATE,
            SystemdGenerator.get_service_file_path(package),
            {
                ...
                'multi_instance': False, # TODO: support for multi-instance
            }
        )

        get_logger().info('Installed {}'.format(SystemdGenerator.get_service_file_path(package)))
...
#            if not dependent_of:
#                continue
#            # TODO: support dependent_of requires other services to be autogenerated
